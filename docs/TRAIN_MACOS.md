...
驗證用的是 EMA 權重，而 EMA 從**模型的初始隨機權重**起步。舊版用固定 decay，
n 步之後仍殘留 `ema_decay^n` 的初始化，短跑會被隨機權重主導：

| 步數 | 殘留的隨機初始化 | 驗證分數 |
|---|---|---|
| 160 步 @ 0.995（舊版） | 45% | 可走性 mIoU 0.16 |
| 同一份權重（非 EMA） | 0% | 可走性 mIoU **0.95** |

現在 decay 改為隨更新次數爬升 `decay * (1 - exp(-updates / ema_warmup_steps))`
（YOLOv5／timm 的標準做法），前幾步幾乎直接複製模型，平滑強度隨著平均累積歷史才變強。
18 步的冒煙實驗開不開 EMA 現在會拿到相同分數，**不必再為了短跑特地關掉**。

`ema_warmup_steps` 預設 2000。真的短到連爬升期都跑不完時，`Trainer` 仍會印出
殘留比例的警告；那時再考慮：
