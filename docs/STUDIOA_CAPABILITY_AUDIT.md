...
> 2026-09-17 新增要求：Stage0 須能在陌生 StudioA 場域無逐鏡頭互動建置；
> [自動化契約、現有依賴與驗收](STAGE0_AUTONOMOUS_NEW_STORE.md)。以下分類盤點保留其原日期。
...
後續已落實 [v1 分類與標註契約](STUDIOA_SCENE_CONTRACT.md)、本機標註包準備／
檢查工具，以及依使用者要求的 [AI 自動標註](STUDIOA_AI_ANNOTATION.md)。
人工標註不再是交付標籤的前置要求；主要模型的分類能力仍需後續訓練，
獨立驗收也不能以教師標籤一致率替代。
