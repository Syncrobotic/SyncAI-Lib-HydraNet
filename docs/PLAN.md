...
8. **No NVDEC backend exists on this box** — raised 2026-08-25. Gate 3's decode leg is
   specified as NVDEC and there is nothing to run it on: no PyNvVideoCodec, no DALI, no
   PyAV, and `~/.local/bin/ffmpeg` (the only one on PATH) reports `vdpau` as its sole
   hwaccel. The current path is a CPU rawvideo pipe, which measured 63–69 streams
   against the 96 required and moves 8.9 GB/s of rgb24 through pipes at target rate.
   Blocks step 3's gate. The choice — PyNvVideoCodec, an ffmpeg built with cuvid, or
   DALI — is an environment decision on a shared box, so it is the user's, not a guess.
