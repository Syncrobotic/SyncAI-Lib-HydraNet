...
The distinction matters more than it looks. `git show b7457c2:docs/RETAIL.md`
measured self-training
...
def main(argv: list[str] | None = None) -> int:
    ...
    for clip, session in zip(args.clips, sessions, strict=True):
        ...
        per_frame: list[tuple[Image.Image, np.ndarray]] = [
            frame_masks(
                proc, model, kept[idx], static, device=args.device, upscale=args.upscale
            )
            for idx in picks
        ]
        shared = None
